...
INSTALLED_APPS = [
    ...
    'payment',  # Payment
    'lemon_admin',
    'corsheaders',   # Backend application creation
]
...
SIMPLE_JWT = {
    'ACCESS_TOKEN_LIFETIME': timedelta(days=1),  # Access token expiration date
    'REFRESH_TOKEN_LIFETIME': timedelta(days=7),  # Refresh token expiration date

    'AUTH_HEADER_TYPES': ('Bearer',),  # Type of authentication header, default is 'Bearer'

    'ROTATE_REFRESH_TOKENS': True,  # Allow refresh tokens
    'BLACKLIST_AFTER_ROTATION': True,  # When the token is refreshed, the old token is invalidated
